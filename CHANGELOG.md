...
- The `timezone` plugin setting is now a searchable dropdown of real timezone identifiers instead of a free-text field, using Grav core's own `Utils::timezones()` provider -- the exact same one core uses for its own `system.timezone` setting, already on the dynamic-callable allowlist by default.

- **Changed:** `started_at`/`ended_at` are now stored as UTC internally, converted from an admin's naive input (interpreted as the plugin's configured `timezone`) on save, and converted back to that same timezone when Admin2 displays them for editing -- fixing a real bug found while testing the `ended_at` auto-fill: with a non-UTC `timezone` configured, the auto-filled value (computed in that timezone) and an admin-entered `started_at` (interpreted with an inconsistent, unrelated default) could be compared incorrectly, producing a false-positive "ended_at must not be earlier than started_at" rejection. The public status page now renders every timestamp as its stored UTC value in a `<time>` element that a small script (`js/status-page.js`) reformats to each visitor's own browser-local time on load, rather than a single site-wide timezone for every viewer.

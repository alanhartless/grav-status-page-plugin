...
/**
 * Pins the single highest-risk mistake this plugin can make: every Flex
 * storage path MUST resolve under `user-data://` and nothing else.
 *
 * On a typical containerized Grav deployment, `user/data` is the part of
 * the install that lives on a persistent volume; `user/plugins` (which is
 * what `plugin://` resolves to) is often image-baked and destroyed on
 * every redeploy. A `plugin://` storage path would lose every category
 * and every incident

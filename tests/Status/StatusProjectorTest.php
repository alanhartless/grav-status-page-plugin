...
/**
 * StatusProjector is this plugin's only genuinely error-prone logic
 * (interval overlap, open-ended-incident handling, day-boundary/DST
 * arithmetic), so this suite intentionally carries the bulk of the
 * plugin's test coverage. Every boundary scenario has its own dedicated
 * test rather than being folded into a bigger "happy path" assertion, so
 * a future regression points at the exact rule that broke.
 *
...
final class StatusProjectorTest extends TestCase
{
    ...
    // -- Public windowStart()/activeInterval(): the rendering layer's
    // "resolved announcements within the window" section must derive its
    // cutoff from the exact same boundary the strip uses, not a second,
    ...
    // -- Malformed input resilience: status-announcements is hand-editable
    // YAML on disk (`user-data://flex-objects/...`), so a single record
    // with an unparseable started_at/ended_at must not take down the
    // public page for every category. The record is skipped -- it

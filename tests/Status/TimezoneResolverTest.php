...
/**
 * Pure resolution of "which timezone decides today" (EPIC-205 D10): plugin
 * config -> Grav's system.timezone -> UTC, read in exactly one place.
 *
 * This is the one piece of Grav-coupled config reading ISSUE-205.3 explicitly
 * deferred to ISSUE-205.4's call site (its own AC notes this split). Kept
 * pure by taking the two already-read config strings as plain arguments
 * rather than touching Grav\Common\Grav::instance() itself -- the plugin's
 * onTwigPageVariables call site does that one read and passes the strings in.
 */
